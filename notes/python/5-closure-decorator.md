...
데코레이터는 기존 함수의 코드를 직접 수정하지 않고, 기존 함수에 새로운 기능이나 동작을 추가하거나 변경할 때 사용하는 방법이다.

데코레이터를 적용할 함수 위에 `@decorator` 형식으로 지정한다.
```python
@decorator
def func():
    ...
...
데코레이터는 일반적으로 함수를 인자로 받아 새로운 함수를 반환하는 함수이다.

예를 들어 기존 함수의 실행 전후에 특정 기능을 추가하고 싶다고 하자.
```python
def decorator(func):
    def wrapper():
        print("함수 실행 전")
    ...
    return wrapper
```
- `func`: 데코레이터가 적용될 기존 함수
- `wrapper`: 기존 함수에 새로운 기능을 추가한 함수
...
def hello(): 
    print("Hello")
```
그대로이지만, 데코레이터를 이용하여 함수 실행 전후에 새로운 기능을 추가할 수 있다.
...
따라서 이후 `hello()`를 호출하면 원래 `hello()`가 직접 호출되는 것이 아니라 데코레이터가 반환한 `wrapper()`가 호출된다.

`wrapper()` 내부에서 원래 함수인 `func()`를 호출하기 때문에 기존 함수의 동작도 함께 실행된다.

실제 함수는 다양한 인자를 받을 수 있으므로 데코레이터에서는 일반적으로 `*args`, `**kwargs`를 사용한다.
...
8
```
여기서 `func(*args, **kwargs)`를 사용하기 때문에 데코레이터가 적용되는 함수가 어떤 인자를 받더라도 전달할 수 있다.
...
def func():
    ...
```
이는 개념적으로 `func = decorator1(decorator2(func))`와 같다. 

함수와 가까운 `@decorator2`가 먼저 적용되고, 그 결과에 `@decorator1`이 적용된다.
